...
> **Développement V6 :** la [PR #14](https://github.com/LeVraiPedro/SkyPortal-Thor/pull/14) a été fusionnée après validation ciblée sur Thor/Spyro’s Adventure et accord utilisateur. `main` inclut la composition corrigée et les correctifs du cycle de vie Dolphin (`12d23a1`), avec CI réussie. Le chantier **V6.0 Bifrost**, autorisé ensuite avec fiabilisation préalable, se poursuit sur `agent/v6-bifrost-integration`. Bifrost officiel **1.3.1 / code 16** a été audité et installé ; sa configuration initiale attend confirmation. L’intégration du compagnon est présente sur la branche et les contrôles locaux ont réussi, mais les LED physiques et leur restauration restent à valider. Elle est facultative, désactivée par défaut, sans changement Dolphin ni permission root supplémentaire. Une commande acceptée ne prouve pas que les LED physiques ont changé. La release publique stable `v0.5.0` reste une paire **API 3** ; la source V6 utilise **API 4**. Ne pas mélanger les APK de ces deux générations. Voir [le suivi courant](docs/PROJECT_STATUS.md), [la roadmap V6](docs/ROADMAP_V6.md), [le contrat LED API 4](docs/V6_LED_API4.md) et [le contrat Bifrost et ses limites](docs/V6_BIFROST.md).
